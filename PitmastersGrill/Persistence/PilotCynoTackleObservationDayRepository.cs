...
{
    public sealed class PilotCynoTackleObservationDayRepository
    {
        private readonly string _databasePath;

        public PilotCynoTackleObservationDayRepository(string databasePath)
        {
            _databasePath = databasePath;
        }

        public void ReplaceDay(string dayUtc, IReadOnlyList<PilotCynoTackleObservationDayRecord> records)
        {
            ...
            using (var deleteCommand = connection.CreateCommand())
            {
                ...
                deleteCommand.CommandText = "DELETE FROM pilot_cyno_tackle_observations_day WHERE day_utc = $dayUtc;";
        ...
        public int ClearAll()
        {
            ...
            command.CommandText = "DELETE FROM pilot_cyno_tackle_observations_day;";
            return command.ExecuteNonQuery();
        }

        public List<CynoHullTackleEvidence> GetRecentTackleEvidenceByCharacterId(string characterId, int maxResults = 10)
        {
            var normalizedCharacterId = NormalizeCharacterId(characterId);
            if (string.IsNullOrWhiteSpace(normalizedCharacterId))
            {
                return new List<CynoHullTackleEvidence>();
            }

            var results = new List<CynoHullTackleEvidence>();
            ...
            while (reader.Read())
            {
                results.Add(new CynoHullTackleEvidence
                {
                    ...
                    TackleModuleTypeId = reader.GetInt32(5),
                    TackleModuleName = reader.IsDBNull(6) ? "" : reader.GetString(6),
                    TackleType = ParseTackleType(reader.IsDBNull(7) ? "" : reader.GetString(7)),
                    QuantityDestroyed = reader.IsDBNull(8) ? 0 : reader.GetInt32(8),
                    QuantityDropped = reader.IsDBNull(9) ? 0 : reader.GetInt32(9),
